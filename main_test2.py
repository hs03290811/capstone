...
# ==========================================
@app.get("/")
def read_root(): return {"Hello": "Server is running. Endpoint: /api/recommend"}
...
def recommend_course(req: GPSRequest):
    ...
    with engine.connect() as conn:
        # ★★★ 1. 시작점 후보군 5개 조회 (반경 100m 내 가까운 순) ★★★
        start_nodes_q = text(f"""
            SELECT id FROM {ACTIVE_TABLE_NAME}_vertices_pgr 
            WHERE ST_DWithin(the_geom, ST_SetSRID(ST_MakePoint(:lon, :lat), 4326), 0.001)
            ORDER BY the_geom <-> ST_SetSRID(ST_MakePoint(:lon, :lat), 4326) LIMIT 5;
        """)
        start_candidates = conn.execute(start_nodes_q, {"lon": req.current_lon, "lat": req.current_lat}).fetchall()
        start_node_ids = [r.id for r in start_candidates]
        
        if not start_node_ids:
            raise HTTPException(status_code=404, detail="시작점 주변 도로 데이터 없음")

        # ★★★ 2. "Round-Robin" 탐색: 시작점 하나당 경로 하나씩만 찾고 넘어감 ★★★
        # 최대 10개까지 수집 후 나중에 3개 선별
        ...
        for start_node in start_node_ids:
            ...
            # 섹터(방향)를 랜덤으로 섞어서, 매번 다른 방향을 먼저 시도하게 함
            sectors = [(0, 120), (120, 240), (240, 360)]
            random.shuffle(sectors)
            
            path_found_for_this_node = False # 이 시작점에서 경로를 찾았는지 여부

            for min_d, max_d in sectors:
                if path_found_for_this_node: break # 찾았으면 다음 시작점으로!
                
                # 해당 방향의 후보지 탐색 (LIMIT 5로 빠르게)
                ...
                for t_node in target_ids:
                    ...
                    try:
                        ...
                        # ★ 중요: 찾았으면 즉시 루프 탈출 -> 다음 시작점 후보로 넘어감 ★
                        path_found_for_this_node = True
                        print(f"✅ 시작점({start_node})에서 경로 확보! 다음 시작점으로...")
                        break 
    ...
    print(f"🚀 최종 {len(final_features)}개 코스 반환 (다양성 보장)")
